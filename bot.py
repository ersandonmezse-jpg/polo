...
def clean_lead_card_text(raw_text: str) -> str:
    """Telegram'dan gelen metinden soru, onay, hata ve durum eklerini temizleyip saf kart metnini bırakır."""
    if not raw_text:
        return ""
    t = str(raw_text)
    split_markers = [
...
def reformat_lead_text(raw_text: str) -> str:
    """Saf metin olarak dönen kayıt kartını şık HTML formatında kalınlaştırılmış etiketlerle yeniden giydirir."""
    cleaned = clean_lead_card_text(raw_text)
    if not cleaned:
        return ""
    if "<b>" in cleaned:
        return cleaned

    t = html.escape(cleaned)
    t = re.sub(r"^(📋\s*Kayıt\s*#\d+)(\s*—\s*)(.*)$", r"<b>\1</b>\2<i>\3</i>", t, flags=re.MULTILINE)
    for label in [
        "🕐 Tarih:",
        "💼 Çalışma Durumu:",
        "🆔 T.C. Numarası:",
        ...
        t = t.replace(escaped_label, f"<b>{escaped_label}</b>")
    return t


def build_record_keyboard(sheet_id: str, row_num: int, is_forwarded: bool = False) -> dict:
    """Kayıt altına yerleştirilecek standart etkileşim butonları (Telegram 64 byte limitine tam uyumlu)."""
    s_id = get_short_sheet_id(sheet_id)
    buttons = [
        [
            {"text": "📝 Not Ekle", "callback_data": f"note_req:{s_id}:{row_num}"},
            {"text": "🔴 Olumsuz", "callback_data": f"st:{s_id}:{row_num}:olumsuz"}
        ],
        [
            {"text": "💳 Kredi Düştü", "callback_data": f"kredi_req:{s_id}:{row_num}"},
            {"text": "📵 Cevapsız", "callback_data": f"st:{s_id}:{row_num}:cevapsiz"}
        ]
    ]
    if is_forwarded:
        buttons.append([
            {"text": "🏁 İşi Bitir & Geri Çek", "callback_data": f"islem_bitti:{s_id}:{row_num}"}
        ])
    else:
        buttons.append([
            {"text": "↗️ Gruba Aktar", "callback_data": f"fwd_menu:{s_id}:{row_num}"}
        ])
...
def _async_process_tc(chat_id: int | str, tc_query: str, reply_to_id: int = None):
    try:
        clean_tc = re.sub(r"[^\d]", "", str(tc_query))
        if not clean_tc or len(clean_tc) != 11:
            send_telegram_message(
                f"⚠️ <code>{html.escape(str(tc_query))}</code> geçerli bir 11 haneli T.C. kimlik numarası değil.",
                chat_id=chat_id,
                reply_to_message_id=reply_to_id
            )
            return

        res = search_by_tc(clean_tc)
        if res:
            card = format_tc_card(res)
            send_telegram_message(card, chat_id=chat_id, reply_to_message_id=reply_to_id)
        else:
            send_telegram_message(f"❌ <code>{clean_tc}</code> numaralı TC kaydı bulunamadı.", chat_id=chat_id, reply_to_message_id=reply_to_id)
    except Exception as e:
        logger.error(f"Async TC sorgu hatası: {e}", exc_info=True)
        send_telegram_message("❌ Sorgu sırasında bir hata oluştu.", chat_id=chat_id, reply_to_message_id=reply_to_id)


def _async_process_gsm(chat_id: int | str, gsm_query: str, reply_to_id: int = None):
    try:
        clean_gsm = re.sub(r"[^\d]", "", str(gsm_query))
        res = search_by_gsm(clean_gsm)
        if res:
            card = format_tc_card(res)
            send_telegram_message(card, chat_id=chat_id, reply_to_message_id=reply_to_id)
        else:
            send_telegram_message(f"❌ <code>{html.escape(gsm_query)}</code> numarasına ait kayıt bulunamadı.", chat_id=chat_id, reply_to_message_id=reply_to_id)
    except Exception as e:
        logger.error(f"Async GSM sorgu hatası: {e}", exc_info=True)
        send_telegram_message("❌ Sorgu sırasında bir hata oluştu.", chat_id=chat_id, reply_to_message_id=reply_to_id)


def _async_process_db_test(chat_id: int | str, reply_to_id: int = None):
    try:
        ok, msg = test_db_connection()
        status_icon = "🟢" if ok else "🔴"
        send_telegram_message(f"{status_icon} <b>VPS Veritabanı Durumu:</b>\n\n{html.escape(msg)}", chat_id=chat_id, reply_to_message_id=reply_to_id)
    except Exception as e:
        logger.error(f"Async DB test hatası: {e}", exc_info=True)
        send_telegram_message(f"🔴 <b>VPS Test Hatası:</b> {html.escape(str(e))}", chat_id=chat_id, reply_to_message_id=reply_to_id)
            ...
            for update in data.get("result", []):
                ...
                if "callback_query" in update:
                    ...
                    if cq_data.startswith("st:"):
                        parts = cq_data.split(":")
                        if len(parts) >= 4:
                            sheet_id = resolve_sheet_id(parts[1])
                            s_id = get_short_sheet_id(sheet_id)
                            row_num = int(parts[2])
                            st_key = parts[3]
                            ...
                            base_text = reformat_lead_text(original_text)
                            ...
                            edit_telegram_message(chat_id, message_id, new_text, reply_markup=build_record_keyboard(s_id, row_num))
                    ...
                    # 1.3) "💳 Kredi Düştü" Tıklandı -> Tutar Seçim Butonları
                        ...
                        amount_keyboard = {
                            "inline_keyboard": [
                                [
                                    {"text": "5.000 TL", "callback_data": f"kredi_amt:{s_id}:{row_num}:5.000 TL"},
                                    {"text": "10.000 TL", "callback_data": f"kredi_amt:{s_id}:{row_num}:10.000 TL"}
                                ],
                                [
                                    {"text": "20.000 TL", "callback_data": f"kredi_amt:{s_id}:{row_num}:20.000 TL"},
                                    {"text": "50.000 TL", "callback_data": f"kredi_amt:{s_id}:{row_num}:50.000 TL"}
                                ],
                                [
                                    {"text": "✍️ Diğer Tutar", "callback_data": f"kredi_custom:{s_id}:{row_num}"},
                                    {"text": "❌ İptal", "callback_data": f"fwd_cancel:{s_id}:{row_num}"}
                                ]
                            ]
                        }
                        clean_card = reformat_lead_text(original_text)
                    ...
                    # 1.5) Tutar Seçildi -> Durumu Güncelle ve SAHA GRUBUNA BİLDİRİM AT
                        ...
                        clean_amt_for_cb = amount_str.replace(" ", "")

                        if saha_chat_id:
                            answer_callback_query(cq_id, f"✅ Kredi kaydedildi: {amount_str}! Saha grubuna iletildi.")
                            saha_msg = (
                            ...
                            iban_keyboard = {
                                "inline_keyboard": [
                                    [{"text": "🏦 IBAN Paylaş", "callback_data": f"iban_req:{s_id}:{row_num}:{chat_id}:{message_id}:{clean_amt_for_cb}"}]
                                ]
                            }
                            send_telegram_message(saha_msg, chat_id=saha_chat_id, reply_markup=iban_keyboard)
                        else:
                            answer_callback_query(cq_id, f"✅ Kredi kaydedildi: {amount_str}! (⚠️ Saha grubu henüz tanımlanmamış)", show_alert=True)
                            send_telegram_message(
                                f"ℹ️ <b>Kredi Durumu Güncellendi:</b> {amount_str}\n"
                                f"⚠️ <i>Saha Grubu henüz tanımlı olmadığı için saha ekibine bildirim iletilemedi.\n"
                                f"Bu grubu saha grubu yapmak için: <code>/saha_grubu</code> yazabilirsiniz.</i>",
                                chat_id=chat_id,
                                reply_to_message_id=message_id
                            )
                    ...
                    # 1.6.B) "🎯 Atış Atıldı" Tıklandı -> Tutar İsteme Menüsü
                        ...
                        atis_amount_kb = {
                            "inline_keyboard": [
                                [
                                    {"text": f"🎯 {default_amount}", "callback_data": f"atis_amt:{s_id}:{row_num}:{default_amount.replace(' ', '')}"},
                                    {"text": "10.000 TL", "callback_data": f"atis_amt:{s_id}:{row_num}:10.000TL"}
                                ],
                                [
                                    {"text": "20.000 TL", "callback_data": f"atis_amt:{s_id}:{row_num}:20.000TL"},
                                    {"text": "50.000 TL", "callback_data": f"atis_amt:{s_id}:{row_num}:50.000TL"}
                                ],
                                [
                                    {"text": "✍️ Özel Tutar Yaz", "callback_data": f"atis_custom:{s_id}:{row_num}"},
                                    {"text": "❌ İptal", "callback_data": f"fwd_cancel:{s_id}:{row_num}"}
                                ]
                            ]
                        }
                        ...
                        edit_telegram_message(
                            chat_id, message_id,
                            reformat_lead_text(original_text) + "\n\n<i>🎯 Lütfen atılan tutarı seçiniz:</i>",
                    ...
                    # 1.6.D) Atış Tutarı Seçildi -> SAHA GRUBUNA GÖNDER (Onay / Düşmedi Butonlarıyla)
                        ...
                        now_time = datetime.now(TURKEY_TZ).strftime("%H:%M")
                        base_txt = reformat_lead_text(original_text)
                        ...
                        if saha_chat_id:
                            saha_confirm_kb = {
                                "inline_keyboard": [
                                    [
                                        {"text": "✅ Onay", "callback_data": f"saha_onay:{s_id}:{row_num}:{chat_id}:{message_id}:{clean_amt}"},
                                        {"text": "🚫 Bloke", "callback_data": f"saha_bloke:{s_id}:{row_num}:{chat_id}:{message_id}:{clean_amt}"},
                                        {"text": "⏳ Düşmedi", "callback_data": f"saha_dusmedi:{s_id}:{row_num}:{chat_id}:{message_id}:{clean_amt}"}
                                    ]
                                ]
                            }
                            send_telegram_message(
                    ...
                    # 1.6.E-2) Sahacı "⏳ Düşmedi" Dedi -> İLK GRUBA BİLGİ VER & SAHAYA "Şimdi Düştü" BUTONU BIRAK
                        ...
                        later_confirm_kb = {
                            "inline_keyboard": [
                                [
                                    {"text": "✅ Para Şimdi Düştü (Onayla)", "callback_data": f"saha_onay:{s_id}:{row_num}:{origin_chat_id}:{origin_msg_id}:{clean_amt}"}
                                ]
                            ]
                        }
                        edit_telegram_message(
                    ...
                    # 1.6.F) Sahacı "✅ Onay" Verdi -> HAKEDİŞ HESABI & İLK GRUBA ONAY BİLDİRİMİ
                        ...
                        next_step_kb = {
                            "inline_keyboard": [
                                [
                                    {"text": "▶️ Devam Et", "callback_data": f"devam_et:{s_id}:{row_num}"},
                                    {"text": "🏁 İşlem Bitti", "callback_data": f"islem_bitti:{s_id}:{row_num}:{clean_amt}"}
                                ]
                            ]
                        }
                    ...
                    # 1.6.G) "▶️ Devam Et" Tıklandı
                        ...
                        edit_telegram_message(
                            ...
                            reply_markup=build_record_keyboard(s_id, row_num)
                        )
                    ...
                    # 1.7) Gruba Aktar Menüsü
                        ...
                        if not groups:
                            answer_callback_query(
                                cq_id,
                                "⚠️ Henüz aktarılacak bir işlem grubu eklenmemiş.",
                                show_alert=False
                            )
                            # Mesaj kartını bilgilendirici ve tek tıkla grup ekleme butonuyla güncelle
                            no_group_kb = {
                                "inline_keyboard": [
                                    [{"text": "➕ Bu Grubu İşlem Grubu Yap", "callback_data": f"fwd_add_here:{s_id}:{row_num}"}],
                                    [{"text": "❌ İptal / Geri", "callback_data": f"fwd_cancel:{s_id}:{row_num}"}]
                                ]
                            }
                            edit_telegram_message(
                                chat_id, message_id,
                            ...
                            group_buttons = []
                            for g in groups:
                                group_buttons.append([{"text": f"👥 {g['name']}", "callback_data": f"fwd_sel:{s_id}:{row_num}:{g['id']}"}])

                            group_buttons.append([
                                {"text": "➕ Bu Grubu da Ekle", "callback_data": f"fwd_add_here:{s_id}:{row_num}"},
                                {"text": "❌ İptal", "callback_data": f"fwd_cancel:{s_id}:{row_num}"}
                            ])
                    ...
                    # 1.7.B) Tek Tıkla Bu Grubu Hedef Grup Yap
                        ...
                        for g in groups:
                            group_buttons.append([{"text": f"👥 {g['name']}", "callback_data": f"fwd_sel:{s_id}:{row_num}:{g['id']}"}])
                        group_buttons.append([{"text": "❌ İptal", "callback_data": f"fwd_cancel:{s_id}:{row_num}"}])
                    ...
                    # 1.8) Hedef Grup Seçildi -> Onay Adımı
                        ...
                        confirm_keyboard = {
                            "inline_keyboard": [
                                [
                                    {"text": "✅ Evet, Gruba Gönder", "callback_data": f"fwd_do:{s_id}:{row_num}:{target_chat_id}"}
                                ],
                                [
                                    {"text": "❌ Vazgeç", "callback_data": f"fwd_cancel:{s_id}:{row_num}"}
                                ]
                            ]
                        }
                        clean_card = reformat_lead_text(original_text)
                    ...
                    # 1.9) Aktarmayı Onayla
                        ...
                        clean_lead_text = reformat_lead_text(original_text)

                        success, target_msg_id = send_telegram_message(
                            clean_lead_text + f"\n\n<i>(Aktaran: {html.escape(user_tag)})</i>",
                            chat_id=target_chat_id,
                            reply_markup=build_record_keyboard(s_id, row_num, is_forwarded=True)
                        )
                        ...
                        if success and target_msg_id:
                            ...
                            edit_telegram_message(chat_id, message_id, updated_orig, reply_markup=build_record_keyboard(s_id, row_num))
                        else:
                            answer_callback_query(
                                cq_id,
                    ...
                    # 1.10) İptal Et
                        ...
                        s_id = get_short_sheet_id(sheet_id)
                        row_num = int(parts[2])
                        clean_text = reformat_lead_text(original_text)
                    ...
                    # 1.11) Tablo Silme Callback'leri
                        ...
                        edit_telegram_message(chat_id, message_id, "❌ İşlem iptal edildi.")

                    elif cq_data == "wipe_all_data":
                        answer_callback_query(cq_id, "Tüm sistem verileri ve mesajları temizleniyor...", show_alert=False)
                        success, msg_resp, cnt = wipe_all_system_data(delete_sheets=False, delete_telegram=True)
                        if success:
                            edit_telegram_message(
                                chat_id, message_id,
                                f"🗑️ <b>Sistemdeki Tüm Veriler & Mesajlar Sıfırlandı!</b>\n\n"
                                f"• <b>{cnt}</b> adet Telegram mesajı gruptan silindi.\n"
                                f"• Tüm lead sayaçları, müşteri geçmişi ve durum kayıtları temizlendi.\n\n"
                                f"<i>Yeniden aktarmak için <code>/aktar</code> veya yeni tablo için <code>/link</code> yazabilirsiniz.</i>"
                            )
                        else:
                            edit_telegram_message(chat_id, message_id, f"❌ Hata: {msg_resp}")

                    elif cq_data == "restore_default_sheet":
                        answer_callback_query(cq_id, "Varsayılan tablo yükleniyor...", show_alert=False)
                        success, msg_resp = restore_default_sheets()
                        if success:
                            edit_telegram_message(
                                chat_id, message_id,
                                f"✅ <b>Varsayılan Tablo (zigiligo) Başarıyla Yüklendi!</b>\n\n"
                                f"Kayıtları gruba aktarmak için <code>/aktar</code> yazabilirsiniz."
                            )
                        else:
                            edit_telegram_message(chat_id, message_id, f"❌ Hata: {msg_resp}")

                    elif cq_data == "reset_counters_act":
                        reset_sheet_last_sent()
                        target_group = str(chat_id)
                        answer_callback_query(cq_id, "Sayaçlar sıfırlandı!", show_alert=False)
                        edit_telegram_message(chat_id, message_id, f"🔄 <b>Aktarım sayaçları sıfırlandı!</b>\nTüm kayıtlar <code>{target_group}</code> sohbetine aktarılıyor...")
                ...
                # ── 2. METİN MESAJLARI & KOMUTLAR ──────────────────────────
                    ...
                    if pending and pending.get("action") == "note":
                        ...
                        base_text = reformat_lead_text(orig_text)
                        now_time = datetime.now(TURKEY_TZ).strftime("%H:%M")
                        new_text = (
                            f"{base_text}\n"
                            f"━━━━━━━━━━━━━━━━━━━━━━\n"
                        ...
                        edit_telegram_message(target_chat_id, target_msg_id, new_text, reply_markup=build_record_keyboard(s_id, row_num))
                        clear_pending_action(user_id)
                    ...
                    # Eğer özel tutar yazıyorsa
                        ...
                        if saha_chat_id:
                            send_telegram_message(f"✅ Kredi tutarı ({amount_str}) kaydedildi ve saha grubuna yönlendirildi.", chat_id=from_chat_id)
                            saha_msg = (
                                f"🔔 <b>YENİ KREDİ ONAYLANDI - SAHA BİLGİLENDİRME</b>\n"
                                f"━━━━━━━━━━━━━━━━━━━━━━\n"
                                f"👥 <b>İlgili Sahacılar:</b> {sahaci_tags}\n"
                            ...
                            iban_keyboard = {
                                "inline_keyboard": [
                                    [{"text": "🏦 IBAN Paylaş", "callback_data": f"iban_req:{s_id}:{row_num}:{target_chat_id}:{target_msg_id}:{clean_amt_for_cb}"}]
                                ]
                            }
                            send_telegram_message(saha_msg, chat_id=saha_chat_id, reply_markup=iban_keyboard)
                        else:
                            send_telegram_message(
                                f"✅ Kredi tutarı ({amount_str}) kaydedildi.\n"
                                f"⚠️ <i>Saha Grubu henüz tanımlanmadığı için saha ekibine bildirim gönderilemedi.\n"
                                f"Bu grubu saha grubu yapmak için: <code>/saha_grubu</code> yazabilirsiniz.</i>",
                    ...
                    # Eğer sahacı IBAN yazıyorsa -> İLK GRUBA İLET
                        ...
                        atis_keyboard = {
                            "inline_keyboard": [
                                [
                                    {"text": "🎯 Atış Atıldı", "callback_data": f"atis_req:{s_id}:{row_num}:{clean_amt_for_cb}"},
                                    {"text": "❌ İşlem Kaçtı", "callback_data": f"islem_kacti:{s_id}:{row_num}"}
                                ]
                            ]
                        }
                    ...
                    # /help veya /yardim Komutu
                        ...
                        help_text = (
                            ...
                            "• <code>/temizle</code> veya <code>/sifirla</code>\n"
                            "  ↳ <i>Sistem sayaçlarını sıfırlar veya gruptaki tüm mesajları temizler.</i>\n\n"
                            "• <code>/durum</code>\n"
                    ...
                    # /link_sil veya /linksil veya link sil Komutu (ÖNCELİKLİ: Silme komutları link eklemeden önce kontrol edilir)
                    is_link_delete = (
                        base_cmd in ["/link_sil", "/linksil", "/tablo_sil", "/tablosil", "link_sil", "linksil", "tablo_sil", "tablosil"]
                        or cmd in ["link sil", "/link sil", "tablo sil", "/tablo sil", "linki sil", "/linki sil", "link sil ", "/link sil "]
                        or cmd.startswith(("/link_sil", "/linksil", "/tablo_sil", "/tablosil", "link sil ", "/link sil ", "tablo sil ", "link_sil ", "linksil "))
                    )

                    if is_link_delete:
                        arg_target = ""
                        for pfx in ["/link_sil", "/linksil", "/tablo_sil", "/tablosil", "/link sil", "link sil", "tablo sil", "link_sil", "linksil"]:
                            if cmd.startswith(pfx):
                                arg_target = text[len(pfx):].strip()
                                break
                        if not arg_target and len(parts) > 1 and parts[0].lower() in ["/link", "link"] and parts[1].lower() == "sil":
                            arg_target = " ".join(parts[2:]).strip()

                        sheets = get_sheets()
                        if not sheets:
                            buttons = [
                                [{"text": "⚡ Varsayılan Tabloyu (zigiligo) Yükle", "callback_data": "restore_default_sheet"}],
                                [{"text": "🗑️ Tüm Sistem Verilerini & Mesajları Sıfırla", "callback_data": "wipe_all_data"}]
                            ]
                            send_telegram_message(
                                "⚠️ <b>Sistemde kayıtlı herhangi bir e-tablo bulunamadı.</b>\n\n"
                                "Hafızada kalan eski kayıtları/mesajları temizlemek veya varsayılan tabloyu yüklemek için aşağıdaki butonları kullanabilirsiniz:",
                                chat_id=from_chat_id,
                                reply_markup={"inline_keyboard": buttons}
                            )
                        elif arg_target:
                            arg_sid = extract_sheet_id(arg_target)
                            sheet_to_del = None
                            for s in sheets:
                                if (
                                    (arg_sid and s.get("id") == arg_sid)
                                    or arg_target.lower() in s.get("name", "").lower()
                            ...
                            if not sheet_to_del and arg_sid:
                                state = load_state()
                                if arg_sid in state:
                                    delete_sheet(arg_sid)
                                    send_telegram_message(
                                        f"🗑️ <b>E-Tablo Verileri Temizlendi!</b>\n\n"
                                        f"🆔 <b>ID:</b> <code>{arg_sid}</code>\n\n"
                                        f"✅ Hafızada kalan tüm veriler ve Telegram mesajları başarıyla temizlendi.",
                                        chat_id=from_chat_id
                                    )
                                    continue
                            ...
                            buttons.append([{"text": "⚠️ Tüm Sistem Verilerini & Mesajları Sıfırla", "callback_data": "wipe_all_data"}])
                    ...
                    elif (base_cmd in ["/link", "link", "/tablo_ekle", "/link_ekle"] or ("docs.google.com/spreadsheets" in text and not is_link_delete)):
                        parts = text.split(maxsplit=1)
                    ...
                    # /sifirla veya /temizle Komutu
                    elif base_cmd in ["/sifirla", "/reset", "/temizle", "/hard_reset", "sifirla", "temizle"]:
                        if "hepsi" in text.lower() or "tum" in text.lower() or base_cmd in ["/temizle", "/hard_reset", "temizle"]:
                            buttons = [
                                [{"text": "🔄 Sadece Sayaçları Sıfırla & Baştan Aktar", "callback_data": "reset_counters_act"}],
                                [{"text": "🗑️ TÜM Verileri & Telegram Mesajlarını Sil", "callback_data": "wipe_all_data"}],
                                [{"text": "❌ İptal", "callback_data": "delsheet_cancel"}]
                            ]
                            send_telegram_message(
                                "⚙️ <b>Sistem Sıfırlama Menüsü:</b>\n\n"
                                "• <b>Sayaçları Sıfırla:</b> Tablodaki verileri gruba baştan tekrar aktarır.\n"
                                "• <b>TÜM Verileri Sil:</b> Gruptaki tüm mesajları siler, hafızayı tamamen temizler.",
                                chat_id=from_chat_id,
                                reply_markup={"inline_keyboard": buttons}
                            )
                        else:
                    ...
                    # /tc veya /sorgu Komutu (VPS Veritabanı Sorgusu - Asenkron Havuz)
                        ...
                        if len(parts) > 1 and parts[1].strip():
                            tc_val = parts[1].strip()
                        elif msg.get("reply_to_message"):
                            rep_text = msg["reply_to_message"].get("text") or msg["reply_to_message"].get("caption") or ""
                            m_tc = re.search(r"\b([1-9]\d{10})\b", rep_text)
                            if m_tc:
                                tc_val = m_tc.group(1)

                        if tc_val:
                            send_telegram_message("🔍 <i>Veritabanında T.C. sorgulanıyor...</i>", chat_id=from_chat_id, reply_to_message_id=msg_id)
                            DB_QUERY_EXECUTOR.submit(_async_process_tc, from_chat_id, tc_val, msg_id)
                        else:
                            send_telegram_message(
                                "ℹ️ <b>T.C. Sorgusu Nasıl Yapılır?</b>\n\n"
                                "• <code>/tc 12345678901</code> şeklinde numara yazarak\n"
                                "• Veya gruptaki bir mesaja yanıt verip sadece <code>/tc</code> yazarak sorgulayabilirsiniz.",
                                chat_id=from_chat_id,
                    ...
                    # /gsm Komutu (Telefon Numarası Sorgusu - Asenkron Havuz)
                        ...
                        gsm_val = None
                        if len(parts) > 1 and parts[1].strip():
                            gsm_val = parts[1].strip()
                        elif msg.get("reply_to_message"):
                            rep_text = msg["reply_to_message"].get("text") or msg["reply_to_message"].get("caption") or ""
                            m_gsm = re.search(r"\b(0?5\d{9})\b", rep_text)
                            if m_gsm:
                                gsm_val = m_gsm.group(1)

                        if gsm_val:
                            send_telegram_message("🔍 <i>Veritabanında GSM sorgulanıyor...</i>", chat_id=from_chat_id, reply_to_message_id=msg_id)
                            DB_QUERY_EXECUTOR.submit(_async_process_gsm, from_chat_id, gsm_val, msg_id)
                        else:
                            send_telegram_message(
                                "ℹ️ <b>GSM Sorgusu Nasıl Yapılır?</b>\n\n"
                                "• <code>/gsm 05321234567</code> şeklinde numara yazarak\n"
                                "• Veya gruptaki bir mesaja yanıt verip sadece <code>/gsm</code> yazarak sorgulayabilirsiniz.",
                                chat_id=from_chat_id,
                                reply_to_message_id=msg_id
                            )
                        continue
                    ...
                    # /db_durum Komutu (VPS Bağlantı Testi - Asenkron Havuz)
                        ...
                        msg_id = msg.get("message_id")
                        send_telegram_message("⏳ <i>VPS Veritabanı bağlantısı test ediliyor...</i>", chat_id=from_chat_id, reply_to_message_id=msg_id)
                        DB_QUERY_EXECUTOR.submit(_async_process_db_test, from_chat_id, msg_id)
                        continue
